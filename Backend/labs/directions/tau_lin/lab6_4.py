# labs/directions/tau_lin/lab6_4.py
...
# Замкнутая система (с годографом Михайлова). Вариант 4
# ==============================
class Lab6_4_TAU_Lin(BaseLab):
    short = "6.4"
    full = "Замкнутая система. Вариант 4"
    note = "Примечание для ЛР6 (Замкнутая система с годографом Михайлова). Вариант 4"
    active_graph = "АЧХ"
    default_params = {
    ...
    def calculate_response(K1, K2, T1, T2, xi, count_of_dots, w_end):
        ...
        p = 1j * omega
        numerator = K1 * (T2 ** 2 * p ** 2 + 2 * xi * T2 * p + 1)
        denominator = (T1 * T2 ** 2 * p ** 3 +
                       (2 * xi * T1 * T2 + T2 ** 2) * p ** 2 +
                       (T1 + 2 * xi * T2) * p +
                       (1 + K1 * K2))
    ...
    def calculate_ACH(K1, K2, T1, T2, xi, count_of_dots, w_end):
        ...
        omega, H = Lab6_4_TAU_Lin.calculate_response(K1, K2, T1, T2, xi, count_of_dots, w_end)
