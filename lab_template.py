...
class GraphWidget(QWidget):
    ...
    def add_graph_line(self, x, y, label, color=None, axis_labels=("W", "Y"), xlim=None, ylim=None, log_x=False):
        if log_x:
        ...
        self.ax.set_xlabel(axis_labels[0], fontdict={'fontsize': 16, 'fontweight': 'bold', 'fontname': 'Segoe UI'}, labelpad=10)
        self.ax.set_ylabel(axis_labels[1], fontdict={'fontsize': 16, 'fontweight': 'bold', 'fontname': 'Segoe UI'}, labelpad=10, rotation=0)
...
class LabWorkTemplate(QWidget):
    def __init__(self, lab_title="Лабораторная работа", parent=None):
        ...
        self.note_label.setStyleSheet("color: #0D47A1; margin: 0px;")
        left_layout.addWidget(self.note_label)

        control_buttons_layout_left = QVBoxLayout()
        ...
        control_buttons_layout_left.addWidget(self.add_graph_button)
        control_buttons_layout_left.addWidget(self.clear_button)
        left_layout.addLayout(control_buttons_layout_left)

        content_layout.addLayout(left_layout, 1)

        right_layout = QVBoxLayout()
        self.function_buttons_layout = QHBoxLayout()
        self.setup_function_selection_buttons()
    ...
    def add_parameter(self, key, label_text, default_value=""):
        """
        Метод для добавления параметров в форму (кнопки для ввода).
        """
        line_edit = QLineEdit()
    ...
    def update_graph_display(self):
        ...
        if self.active_function in self.stored_curves:
            ...
            self.graph_widget.apply_custom_design()
    ...
    def update_function_buttons_style(self):
        for btn in self.function_buttons:
            if btn.text() == self.active_function:
                btn.setStyleSheet("background-color: #0D47A1; color: white; font-family: 'Segoe UI';")
            else:
                btn.setStyleSheet("")
